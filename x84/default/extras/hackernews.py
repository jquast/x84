...
def articlereader(article):
    ...
    for line in article:      # the string list named text will be zerobased
        if len(line) > 79:    # split line if it's longer than 79 columns.. when html2text screws up
            tempstring = []
        ...
        if dirty == True:
            ...
            for i in range (0, term.height-1):
                if len(text) > i+offset:
                    echo(term.move_x(0)+term.clear_eol+term.move_x((term.width/2)-40)+text[i+offset])
    ...
    while 1:
        ...
        if keypressed == 'q' or keypressed == 'Q' or keypressed == term.KEY_ESCAPE:
            ...
            if (offset+amount) < len(article):
                ...
                if len(article) < amount: # checks wheter the article has fewer lines than the screen or not..
                     offset = 0
        elif keypressed == term.KEY_UP:
            if offset > amount:
                offset = offset - amount
            ...
            echo(term.move(term.height, 0)+term.clear_eol+term.white+'choose your article no#: ')
            le = LineEditor(10)
            le.colors['highlight'] = term.cyan
            inp = le.read()
            if inp.isnumeric() and int(inp) < len(article):
                echo(term.clear+term.yellow+'fetching the latest news just for you..'+term.normal)
                ...
                h.escape_all = True
                h.body_width = 79
                req = urllib2.Request(choosenurl, headers={ 'User-Agent': 'Mozilla' }) # identify as mozilla
                try:
                    text = urllib2.urlopen(req, timeout=10).read()
                except socket.timeout, e:
                    echo(term.clear()+term.yellow+'request timed out.. try again.')
                    waitprompt()
                except urllib2.URLError as e:
                    echo(term.clear()+term.yellow+'faulty http link.. try again.')
                    waitprompt()
                else:
