...
from x84.bbs import getsession, echo, getch, getterminal, showart
import os
from os import walk
...
def helpscreen():
    ...
    text.append(u'')
    text.append(term.bold_white + term.underline +
                u'Key bindings for the navigator:' + term.normal)
    text.append(u'(Q/Escape) to quit.')
    text.append(u'(Up/Dn/Right/Left/Pgup/Pgdn/Enter) to navigate.')
    text.append('')
    text.append(term.bold_white + term.underline +
                u'Key bindings for the file viewer:' + term.normal)
    text.append(u'(Q/Escape/Enter) to return.')
    text.append(u'(Up/Dn/Right/Left/Pgup/Pgdn) to navigate.')
    text.append('')
    text.append(
        term.bold_white + term.underline + 'General key bindings:' + term.normal)
...
# if the variable lightbar is negative the lightbar will be invisible
def redrawlightbar(filer, lighty, lightx, lightbar, start, antalrader):
    term = getterminal()
    echo(term.move(lighty, lightx))

    for i in range(0, 10):
        # erases 45 char. dont want to use clreol. So filenames/directories can
        # be 45 char.
        echo(term.move(lighty + i - 1, lightx) +
             '                                             ')

    i2 = 0
    for i in range(start, start + antalrader):
        if i2 == lightbar:
            echo(term.move(lighty + i - start - 1, lightx) +
                 term.blue_reverse + filer[i] + term.normal)
        else:
            echo(
                term.move(lighty + i - start - 1, lightx) + term.white + filer[i])
        i2 = i2 + 1
...
#********** default variables for you to change ! *************
    ...
    # the folder where you store your textfiles..
    ...
    while 1:
        ...
        if keypressed == term.KEY_LEFT or keypressed == term.KEY_PGUP:
            ...
            if offset < 0:
                ...
                lightbarpos = 0
            redrawlightbar(
                filer, lighty, lightx, lightbarpos, offset, antalrader)
        ...
        if keypressed == term.KEY_UP and lightbarpos + offset > -1:
            echo(term.white + term.move(lighty + lightbarpos - 1, lightx) +
                 filer[lightbarpos + offset])  # restore colour on the old coordinate
        ...
        if keypressed == term.KEY_ENTER:
            ...
            if filer[lightbarpos + offset][-1:] == '/':
                ...
                filer = getfilelist(currentfolder)
# oldlightbarpos = lightbarpos # saves the location of the lightbar in
# previous directory
                ...
                if len(filer) > max_amount_rows:
                    ...
                    # stops the script from printing none-existing rows
                    antalrader = len(filer)
                ...
                for i in range(2, 50):  # support folders up to 50 characters..
                    if currentfolder[len(currentfolder) - i] == '/':
                        ...
                        offset = 0
                        lightbarpos = 0
# lightbarpos = oldlightbarpos # restores the lightbarpos previously used
