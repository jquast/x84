...
databasename = 'votingbooth' # change this if you wish to use an alternate database file
...
def delete_question(questionnumber):
    ...
    if inp == 'a': # delete answer alternative..
        echo (term.clear)
        echo (term.white+questions[questionnumber]+term.move_x(max(0,term.width-12))+' index: '+str(index[questionnumber])+'\r\n\r\n')
        for i in range (0,amount_of_alternatives[questionnumber]):
...
def main():
    ...
    while True: 
        ...
        for line in showart(os.path.join(os.path.dirname(__file__),'art','vote.ans')):
