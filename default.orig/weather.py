...
def main(zipcode=''):
  ...
  if not zipcode:
    ...
    while True:
      ...
      if len(lookup) == 1:
        ...
        # lookup selection
        llb = lightclass(ansiwin(h=19,w=55,y=5,x=12))
        llb.interactive = True
        llb.update (picker, refresh=False)
        if len(picker) < 15:
          llb.resize (len(picker)+2)
        else:
          llb.refresh ()
        llb.ans.lowlight (partial=True)
        llb.interactive = False
        llb.ans.title('-< CtRl-X: CANCEl >-')
        choice = llb.run ()
        llb.ans.noborder ()
        llb.ans.fill ()
        if llb.exit:
  ...
  flb = lightclass(ansiwin(h=10,w=11,y=9,x=6))
  flb.interactive = True
  for entry in fc:
    flb.add (str(entry['DayCode']))

  # weather data window
  pager = paraclass(ansiwin(h=19,w=57,y=5,x=20), xpad=2, split=8, ypad=2)
  ...
  def refresh_windows():
    # clear
    echo (cls() + color() + cursor_show())
    flb.ans.lowlight (partial=True)
    flb.refresh ()
    pager.ans.lowlight (partial=True)
    pager.ans.title (color(RED) + '-< ' + attr(INVERSE) + title + color() + color(RED) + ' >-')
    pager.ans.title (color(RED) + '-< ' + attr(INVERSE) + 'q:QUit, UP/dOWN: fORECASt' + color() + color(RED) + ' >-', align='bottom')
