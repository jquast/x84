...
__license__ = "ISC"
__version__ = "$Id: ansiwin.py,v 1.12 2009/05/17 21:02:35 dingo Exp $ #"
...
class BaseWindow:
  """
  BaseWindow is a themable base class for all user interfaces.

  Window themes may be modified by sending dictionary keys
  such changes to L{setTheme}, affecting highlight/lowlightion
  action colors, border characters, or redefine then in derived
  classes.

  @cvar glyphs: dictionary store of character values:
    - border: 'top-left', 'bottom-left', 'top-right', 'bottom-right',
      'vertical', and 'horizontal'.
    - fill: fill character, default background character (great for debug)
    - erase: erase character, often set for gui debugging and placement

  @cvar colors: dictionary store of ansi sequences, usually a return value of L{ansi.color}
    - highlight: highlighted characters, usualy C{color(REVERSE)}
    - lowlight: in entry selection, entries that are not currently selected
    - active: color of active window border
    - inactive: color of inactive window border

  @todo: support for 2-dimensional [row[col]] art for borders. (got art? :P)
  """

  # set to false for less screen draws, rule of thumb is,
  # if you can make it less pretty but work with 9600,
  # or vice-versa, use this bool
  highdef= True
  ...
  def setTheme(self, theme):
    ...
    if not type(theme.values()[0]) == types.DictionaryType:
      ...
      for col in range (0, self.w):
        ...
        if (col == 0) or (col == self.w -1):
          if (row == 0) and (col == 0):
            ...
            # bottom left
            echo (self.pos(col, row) + glyphs['bottom-left'])
          elif (row == 0):
            # top right
            echo (self.pos(col, row) + glyphs['top-right'])
          elif (row == self.h -1):
            # bottom right
            echo (self.pos(col, row) + glyphs['bottom-right'])
          elif not self.partial:
            # vertical line
            echo (self.pos(col, row) + glyphs['vertical'])
        elif (row == 0):
          echo (self.pos(col, row) + glyphs['horizontal'] *(self.w -2) +glyphs['top-right'])
          # top row
          break
        elif (row == self.h -1):
          # bottom row
          echo (self.pos(col, row) + glyphs['horizontal'] *(self.w -2) +glyphs['bottom-right'])
          break
    if color:
      echo (ansi.color())
