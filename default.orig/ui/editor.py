...
$Id: editor.py,v 1.1 2009/05/17 20:59:45 dingo Exp $
...
class HorizEditor(InteractiveAnsiWindow):
  ...
  def writeString(self, data, col=-1):
    ...
    if col == -1:
      col = self.col
    echo (self.pos(x=self.xpad +col, y=1) +data)
  ...
  def add(self, ch, refresh=True):
    ...
    """
    if self.max and len(self.content) == self.max:
  ...
  def fixate(self, mode=0):
    """ Fixate cursor at current position in window,
        ...
        @return: True
    """
    echo (self.pos(x=self.xpad +self.col +mode, y=1))
    return True
