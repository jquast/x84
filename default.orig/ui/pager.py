...
$Id: pager.py,v 1.13 2009/05/17 21:02:35 dingo Exp $
...
class ParaClass(InteractiveAnsiWindow):
  ...
  def update(self, data='', refresh=True):
    """ Update content buffer. Lines wrap to new row at newlines, or word
    ...
    while data.strip() or remaining.strip():
      ...
      if newline and ansilen(data[:nlpos], max=self.visibleWidth) <= self.visibleWidth:
        remaining = data[nlpos+len(nlseq):] +remaining
        data = data[:nlpos]
      else:
        newline = False
      if newline or ansilen(data, max=self.visibleWidth) <= self.visibleWidth:
      ...
      data = data[:pos]
    return refresh and self.refresh ()
  ...
  def end(self, silent=False):
    ...
    if (len(self.content) -self.visibleHeight < 0):
      ...
      return self.refresh ()
    return not silent or  self.bell ('cannot end: at bottom')
