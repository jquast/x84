...
def describe(s):
  ...
  if not s.terminals:
    ...
    for n, t in enumerate(s.terminals):
      ...
      if 'sysop' in user.groups and hasattr(t, 'char'):
...
def main():
  ...
  # online users
  ulist = lightclass (ansiwin(14, 12, 6, 5))
  ...
  # user info
  uinfo = paraclass(ansiwin(15, 53, 4, 20), ypad=1, xpad=2)

  # key help window
  keys = paraclass(ansiwin(7, 33, 18, 25), xpad=1)
  ...
  while not ulist.exit:
    ...
    if handles != ohandles and not refresh:
      ...
      refresh = False
      ulist.moved = True
      uinfo.ans.lowlight (partial=False)
      ulist.ans.lowlight (partial=True)
      ulist.update (handles)
    ...
    if remote and ulist.lastkey == 'H':
      if not 'sysop' in user.groups \
      and remote.handle != user.handle:
