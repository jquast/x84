...
def main(msgs):
  ...
  pager = paraclass(ansiwin(18,54,6,13), xpad=2, ypad=2)
  pager.interactive = True
  lr = PrevNextClass((60,24), NEXT)
  lr.highlight = color(BROWN) + color (INVERSE)
  lr.lowlight = color(NORMAL) + color(GREY)
  lr.interactive = True
  ...
  def noclear():
    echo (pos(30, 24) + color() + cl())

  def status(text='(^Y) Commands', xy=(30,24)):
    echo (pos(*xy) + color() + cl() + text)
    lr.refresh ()
  ...
  def refresh():
    ...
    pager.ans.title(color(*DARKGREY) + '-< ' + color() + msg.subject + color(*DARKGREY) + ' >-')
    ...
    pp += color(*DARKGREY) + ']'
    pager.ans.title(color(*DARKGREY) + '-< ' + ur + color() + '#' + color() + str(n+1) + ' of ' + str(len(msgs)) + color(*DARKGREY) + ' ' + pp + ' >-',align='bottom')
  ...
  while True:
    ...
    while True:
      ...
      if k in [KEY.LEFT, KEY.RIGHT, 'h', 'l']:
        ...
        lb = lightclass(ansiwin(10,17,10,27))
        lb.alignment = 'center'
        ...
        if lb.exit:
          ...
          if msg.public:
            ...
            msg.set('read', False)
          status (xy=(28,24), text='MESSAGE hAS bECOME UNREAd!')
        elif option == 'mark read':
          if msg.public:
            msg.set('read', [handle()])
          else:
            msg.set('read', timenow())
          status (xy=(28,24), text='MESSAGE hAS bECOME REAd!')
        elif option == 'mark public':
          msg.set('public', True)
          msg.set('read', [])
          status (xy=(20,24), text='MESSAGE hAS bECOME PUbliC ANd UNREAd!')
        elif option == 'mark private':
          msg.set('public', False)
          msg.set('read', False)
          status (xy=(20,24), text='MESSAGE hAS bECOME PRiVAtE ANd UNREAd!')
        elif option == 'soft delete':
          msg.delete (killer=handle())
          status (xy=(20,24), text='MESSAGE hAS bEEN MARkEd dElEtEd!')
          remove (n)
        elif option == 'hard delete':
          msg.delete (force=True, killer=handle())
          status (xy=(20,28), text='MESSAGE hAS bEEN dElEtEd!')
          remove (n, hard=True)
        elif option == 'reply':
